...
function ChangeTag({ value }: { value: number }) {
  ...
  return (
    <span className={`font-mono text-xs font-bold tabular-nums ${positive ? 'text-emerald-400' : 'text-rose-400'}`}>
      {positive ? '+' : ''}
      {(value * 100).toFixed(2)}%
    </span>
  );
...
function MoverListCard({ title, rows }: MoverListCardProps) {
  return (
    <div className="rounded-lg border border-neutral-800 bg-neutral-900">
      <div className="border-b border-neutral-800 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-neutral-300">
        {title}
      </div>
      <div className="divide-y divide-neutral-800/70 px-2">
        {rows.map((r, i) => (
          <Link
            ...
            className="flex min-w-0 items-center gap-1.5 py-1.5 text-xs hover:bg-neutral-800/50"
          >
            <span className="w-3 shrink-0 text-right text-[10px] text-neutral-600">{i + 1}</span>
            <span className="w-14 shrink-0 truncate font-mono font-semibold text-neutral-200">{r.symbol.replace('.JK', '')}</span>
